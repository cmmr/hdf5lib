...
R. The setup to use `hdf5lib` with `cpp11` is very similar to the `Rcpp`
setup.

1.  **`DESCRIPTION`**: Add `cpp11` to the `LinkingTo` field.
2.  **`src/Makevars`**: The file remains identical to the one shown in
...
call the function from R just as you would with an `Rcpp`-based package.
