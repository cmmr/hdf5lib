...
> background pool of worker threads for parallel compression. Tearing
> this down via `hdf5lib_destroy_all_filters()` joins and destroys those
> threads.
>
...
> locking overhead. Instead, it is highly recommended to tie
> registration to your R package’s lifecycle using `.onLoad` and
> `.onUnload` hooks.
...
level). Below are the expected parameters for each bundled filter.

### GZIP / Deflate (`H5Z_FILTER_DEFLATE`)

Instead of `H5Pset_filter`, you can apply this conveniently using
`H5Pset_deflate(plist_id, level)`. \* **Elements (`cd_nelmts`)**: 1 \*
**`cd_values[0]`**: Compression level from `0` (no compression) to `9`
(maximum compression). Default is usually `6`.

### SZIP (`H5Z_FILTER_SZIP`)

You can apply it conveniently using
`H5Pset_szip(plist_id, options_mask, pixels_per_block)`. \*
**Restriction:** Numeric data only. Does not support strings or variable
length types. The chunk size (in elements) must be an exact multiple of
the block size. \* **Elements (`cd_nelmts`)**: 2 \* **`cd_values[0]`**:
Options mask (e.g., `H5_SZIP_NN_OPTION_MASK` or
...
*Cross-Language Compatibility Note:* Datasets written in C or R using
Blosc2 (along with its internal codecs like Snappy, ZFP, and NDLZ) via
`hdf5lib` are fully interoperable. They can be seamlessly read by Python
ecosystems (e.g., using `h5py` equipped with the `hdf5plugin` library).
