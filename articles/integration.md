...
`hdf5lib` using `Rcpp`, `cpp11`, and R’s native C API.
...
Using `Rcpp` is the most common way to link against `hdf5lib`. Make sure
`LinkingTo: hdf5lib, Rcpp` is in your `DESCRIPTION` file, and
`src/Makevars` is set up with the compiler and linker flags.

The following example creates an HDF5 file and uses `hdf5lib`’s bundled
Zstandard plugin to compress the data.
...
R. The setup to use `hdf5lib` with `cpp11` is very similar. Add `cpp11`
to the `LinkingTo` field and retain the same `src/Makevars`.
...
without the overhead of creating a full R package or pulling in C++
abstractions. You can do this using R’s native `.Call` interface,
compiling the C code into a shared library on the fly and loading it
