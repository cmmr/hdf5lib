...
#define DEFAULT_BLOCK_SIZE (1 << 30) /* 1 GiB blocks for large R datasets */
...
static size_t lz4_filter(
  ...
  /* Compression Path */
    ...
    size_t nBlocks = (nbytes == 0) ? 0 : (nbytes - 1) / blockSize + 1;
    ...
    H5free_memory(*buf); *buf = outBuf; *buf_size = totalOut;
    return totalOut;
  }
}

const H5Z_class2_t lz4_class = { H5Z_CLASS_T_VERS, H5Z_FILTER_LZ4, 1, 1, "lz4", NULL, NULL, lz4_filter };
