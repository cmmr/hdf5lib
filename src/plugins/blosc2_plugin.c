...
static size_t blosc2_filter_function(
  ...
  if (!(flags & H5Z_FLAG_REVERSE)) {
    ...
    /* 1D Linear Chunking */
      ...
      if (status < 0 || !needs_free) {
        blosc2_schunk_free(schunk); blosc2_free_ctx(cctx);
        PUSH_ERR("blosc2_filter: Cannot convert super-chunk to buffer");
      }

      /* Safely copy into HDF5-managed memory */
      outbuf = H5allocate_memory((size_t)status, 0);
      if (!outbuf) {
        free(tmp_out); blosc2_schunk_free(schunk); blosc2_free_ctx(cctx);
        PUSH_ERR("blosc2_filter: Memory allocation failed");
      }
      memcpy(outbuf, tmp_out, (size_t)status);
      free(tmp_out);
  ...
  /* ----- Decompression Path ----- */
    ...
    if (blosc2_meta_exists(schunk, "b2nd") >= 0 || blosc2_meta_exists(schunk, "caterva") >= 0) {
      ...
      if (b2nd_from_schunk(schunk, &array) < 0) PUSH_ERR("blosc2_filter: Cannot create B2ND array");
      ...
      if (!outbuf) { b2nd_free(array); PUSH_ERR("blosc2_filter: Cannot allocate decompression buffer"); }

      if (b2nd_get_slice_cbuffer(array, start, stop, outbuf, stop, (int32_t)size) < 0) {
        H5free_memory(outbuf); b2nd_free(array);
    ...
    /* 1D Linear Decompression */
      ...
      if (cbytes < 0) { blosc2_schunk_free(schunk); PUSH_ERR("blosc2_filter: Cannot get chunk from super-chunk"); }
  ...
  if (status > 0) {
    /* Handle incompressible data edge case on write */
    if (!(flags & H5Z_FLAG_REVERSE) && status >= nbytes) {
      H5free_memory(outbuf);
      blosc2_destroy();
      return 0;
    }
