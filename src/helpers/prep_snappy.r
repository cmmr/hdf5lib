# Use this file to download and minify the latest version of snappy.
# The output of this script should be saved to src/snappy-VER.tar.gz and
...
# Download and extract the snappy codebase
baseurl <- "https://github.com/google/snappy/archive/refs/tags/"
url     <- paste0(baseurl, VER, '.tar.gz')
tarfile <- file.path('src', paste0("snappy-", VER, ".tar.gz"))
exdir   <- file.path('src', paste0("snappy-", VER))
cached  <- file.path('src', paste0("snappy-", VER, "-orig.tar.gz"))
...
utils::untar(
  ...
  files   = paste0(paste0("snappy-", VER, "/"), c(
    'COPYING', 'snappy.cc', 'snappy-c.cc', 'snappy-sinksource.cc', 
    'snappy-stubs-internal.cc', 'snappy.h', 'snappy-c.h', 'snappy-internal.h', 
...
# # Apply patches
# cat('Applying snappy patches files...\n')
# patch_dir   <- paste0("src/patches/snappy-", VER)
...
# Create the snappy tarball that will ship with hdf5lib.
cat(paste0("Creating 'snappy-", VER, ".tar.gz'...\n"))
invisible(file.remove(tarfile))
setwd('src')
utils::tar(
  tarfile = paste0("filters/snappy-", VER, ".tar.gz"), 
  files   = paste0("snappy-", VER), 
