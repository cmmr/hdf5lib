# Use this file to download and minify the latest version of liblzf.
# The output of this script should be saved to src/liblzf-VER.tar.gz and
...
# Download and extract the liblzf codebase
baseurl <- "https://dist.schmorp.de/liblzf/Attic/"
url     <- paste0(baseurl, 'liblzf-', VER, '.tar.gz')
tarfile <- file.path('src', paste0("liblzf-", VER, ".tar.gz"))
exdir   <- file.path('src', paste0("liblzf-", VER))
cached  <- file.path('src', paste0("liblzf-", VER, "-orig.tar.gz"))
...
utils::untar(
  ...
  files   = paste0(paste0("liblzf-", VER, "/"), c(
    "LICENSE", "lzf.h", "lzf_c.c", "lzf_d.c", "lzfP.h" )))



# Apply patches
cat('Applying liblzf patches files...\n')
patch_dir   <- paste0("src/patches/liblzf-", VER)
...
# Create the liblzf tarball that will ship with hdf5lib.
cat(paste0("Creating 'liblzf-", VER, ".tar.gz'...\n"))
invisible(file.remove(tarfile))
setwd('src')
utils::tar(
  tarfile = paste0("filters/liblzf-", VER, ".tar.gz"), 
  files   = paste0("liblzf-", VER), 
