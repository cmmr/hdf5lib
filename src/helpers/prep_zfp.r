# Use this file to download and minify the latest version of zfp.
# The output of this script should be saved to src/zfp-VER.tar.gz and
...
# Download and extract the zfp codebase
baseurl <- "https://github.com/llnl/zfp/releases/download/"
url     <- paste0(baseurl, '/', VER, '/zfp-', VER, '.tar.gz')
tarfile <- file.path('src', paste0("zfp-", VER, ".tar.gz"))
exdir   <- file.path('src', paste0("zfp-", VER))
cached  <- file.path('src', paste0("zfp-", VER, "-orig.tar.gz"))
...
unlink(recursive = TRUE, expand = FALSE, x = c(
  file.path(exdir, setdiff(list.files(exdir, recursive = FALSE), c('LICENSE', 'include', 'src'))),
  list.files(file.path(exdir, 'include'), '\\.hpp$', full.names = TRUE, recursive = TRUE),
  file.path(exdir, 'src', c('cuda_zfp', 'CMakeLists.txt', 'Makefile')) ))
...
# Create the zfp tarball that will ship with hdf5lib.
cat(paste0("Creating 'zfp-", VER, ".tar.gz'...\n"))
invisible(file.remove(tarfile))
setwd('src')
utils::tar(
  tarfile = paste0("filters/zfp-", VER, ".tar.gz"), 
  files   = paste0("zfp-", VER), 
