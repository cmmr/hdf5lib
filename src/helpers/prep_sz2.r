# Use this file to download and minify the latest version of SZ.
# The output of this script should be saved to src/SZ-VER.tar.gz and
...
# Download and extract the SZ codebase
baseurl <- "https://github.com/szcompressor/SZ2/releases/download/"
url     <- paste0(baseurl, 'v', VER, '/SZ-', VER, '.tar.gz')
tarfile <- file.path('src', paste0("SZ-", VER, ".tar.gz"))
exdir   <- file.path('src', paste0("SZ-", VER))
cached  <- file.path('src', paste0("SZ-", VER, "-orig.tar.gz"))
...
# dput(substr(grep('/sz/', utils::untar(tarfile, list=TRUE), fixed = TRUE, value = TRUE), 16, 100))

cat("Decompressing '", tarfile, "'...\n")
utils::untar(
  tarfile = tarfile,
  exdir   = dirname(tarfile),
  files   = paste0("SZ-", VER, "/sz/", c('include', 'src')))

# Remove Fortran Interfaces & C Wrappers:
unlink(paste0("src/SZ-", VER, "/sz/", c(
  'include/szf.h', 'src/rwf.c', 'src/szf.c',
  'src/rw_interface.F90', 'src/sz_interface.F90' )))
...
# Create the SZ tarball that will ship with hdf5lib.
cat(paste0("Creating 'SZ-", VER, ".tar.gz'...\n"))
invisible(file.remove(tarfile))
setwd('src')
utils::tar(
  tarfile = paste0("filters/SZ-", VER, ".tar.gz"), 
  files   = paste0("SZ-", VER), 
