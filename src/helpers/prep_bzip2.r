# Use this file to download and minify the latest version of bzip2.
# The output of this script should be saved to src/bzip2-VER.tar.gz and
...
# Download and extract the bzip2 codebase
baseurl <- "https://sourceware.org/pub/bzip2/"
url     <- paste0(baseurl, 'bzip2-', VER, '.tar.gz')
tarfile <- file.path('src', paste0("bzip2-", VER, ".tar.gz"))
exdir   <- file.path('src', paste0("bzip2-", VER))
cached  <- file.path('src', paste0("bzip2-", VER, "-orig.tar.gz"))
...
utils::untar(
  ...
  files   = paste0(paste0("bzip2-", VER, "/"), c(
    "LICENSE", "bzlib.h", "bzlib_private.h", "blocksort.c", "bzlib.c", 
    "compress.c", "crctable.c", "decompress.c", "huffman.c", "randtable.c" )))
...
# Create the bzip2 tarball that will ship with hdf5lib.
cat(paste0("Creating 'bzip2-", VER, ".tar.gz'...\n"))
invisible(file.remove(tarfile))
setwd('src')
utils::tar(
  tarfile = paste0("filters/bzip2-", VER, ".tar.gz"), 
  files   = paste0("bzip2-", VER), 
