...
/* High Precision Float Support (GCC/Clang Extension) */
...
/* ========================================================================== */
/* We statically link zlib, so we can guarantee these are present */
...
/* ========================================================================== */
/* Strategy: If a header exists, assume its standard features are available.
   We explicitly UNDEF broken features in the Platform Specifics section below. */
...
/* --- System Headers --- */
    ...
    /* Note: MinGW has the header but lacks the specific structs/flags HDF5 needs */
    ...
    /* These are GNU/BSD extensions, not C99, but common on R platforms */
    ...
    /* Reentrant qsort is a common extension */
...
/* --- Windows Specifics (Rtools / MinGW) --- */
    ...
    /* * FEATURE EXCLUSIONS
     * The following features were detected by header checks above,
     * but are known to be broken, missing, or unsupported on MinGW/Windows.
     */

    /* Threading: We use Pthreads (detected above), so explicitly deny Win32 threads */
...
#else
    /* POSIX builds usually keep all the features enabled above. */
