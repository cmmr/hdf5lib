...
# The output of this script should be saved to src/blosc-VER.tar.gz and
...
cat(paste0("Creating 'blosc-", VER, ".tar.gz'...\n"))
invisible(file.remove(tarfile))
setwd('src')

origdir <- paste0("c-blosc-", VER)
shipdir <- paste0("blosc-", VER)
tarfile <- paste0(shipdir, ".tar.gz")

if (file.exists(tarfile)) file.remove(tarfile)
file.rename(from = origdir, to = shipdir)
